...
export function parseRetryMs(text, nowMs = Date.now()) {
  const t = text || '';
  ...
  // 4) clock-time reset — the format Anthropic ACTUALLY uses in usage-limit messages, e.g.
  //    "Your limit will reset at 3pm", "5-hour limit reached ∙ resets 3:45pm", "resets at 10:30 PM".
  //    We compute ms until that wall-clock time (assumed local), rolling to tomorrow if it's already
  //    past. Optional timezone tokens (UTC/parenthetical zone) are tolerated but not re-based — the
  //    probe loop absorbs any sub-hour zone skew, and this is still vastly better than blind backoff.
  m = /resets?(?:\s+at)?\s+(\d{1,2})(?::(\d{2}))?\s*([ap]\.?m\.?)/i.exec(t);
  if (m) {
    let hour = Number(m[1]);
