...
// Engine-only token spend (excludes the user's interactive sessions). This is the correct numerator
// for the PER-CYCLE cost estimate — windowSpend().spent is account-wide (measuredWeeklyTokens includes
// the user's own Claude.ai usage) and dividing THAT by engine cycles inflates the per-cycle estimate
// by 10-50×, which crushes cyclesLeft and the adaptive cadence to a near-halt. Use windowSpend().spent
// for "how much of the quota is gone" (headroom); use THIS for "what will one engine cycle cost".
export function engineTrackedTokens(state) {
  const u = (state && state.usage) || {};
...
// The mean is ENGINE-tracked tokens per ENGINE cycle — never the account-wide measured total.
export function estimateNextCycleTokens(state, config) {
  const g = governorConfig(config);
  const { cycles } = windowSpend(state);
  const engineTracked = engineTrackedTokens(state);
  const avg = cycles > 0 && engineTracked > 0 ? engineTracked / cycles : g.minCycleTokens;
