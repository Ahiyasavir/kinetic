...
//
// PLUGIN SCORING (U-44): when config.scoring.plugin is set, the plugin's scoreTask function replaces
// the built-in formula. Call loadScoringPlugin(config, autopilotDir) at startup to pre-load and cache
// it; otherwise scoreTask falls back to the built-in implementation (fully backward compatible).
...
const SCORE_DIR = path.dirname(fileURLToPath(import.meta.url));

// Module-level plugin registry. Null = no plugin loaded; use built-in scoring.
let _scoringPlugin = null;

/**
 * Register a custom scoring function directly (synchronous). The function receives
 * (task, state, config) and returns a plain number. This registration path is used when
 * the caller already has the module reference (tests, programmatic use).
 * @param {Function} fn
 */
export function registerScoringPlugin(fn) {
  if (typeof fn !== 'function') throw new TypeError('scoring plugin must export a scoreTask function');
  _scoringPlugin = fn;
}

/**
 * Async: read config.scoring.plugin (a path relative to autopilotDir), dynamically import the
 * module, and register its scoreTask export. No-ops when the config key is absent.
 * @param {object} config   — resolved engine config
 * @param {string} [autopilotDir] — directory of the autopilot/ folder (default: ../lib/)
 */
export async function loadScoringPlugin(config, autopilotDir) {
  const pluginRel = config && config.scoring && config.scoring.plugin;
  if (!pluginRel) return;
  const dir = autopilotDir || path.resolve(SCORE_DIR, '..');
  const pluginPath = path.resolve(dir, pluginRel);
  const mod = await import(pluginPath);
  if (typeof mod.scoreTask !== 'function') {
    throw new TypeError(`scoring plugin at "${pluginPath}" must export a scoreTask function`);
  }
  _scoringPlugin = mod.scoreTask;
}
...
export function scoreTask(task, config) {
  // Delegate to a registered scoring plugin when available.
  if (_scoringPlugin) {
    const pluginTotal = _scoringPlugin(task, null, config);
    const total = typeof pluginTotal === 'number' ? pluginTotal : 0;
    return { total, contrib: {}, reason: `(plugin scorer)` };
  }
