...
export async function discardCycle(cwd, integration, cycleBranch) {
  await git(cwd, ['reset', '--hard'], { allowFail: true });
  // `-e kinetic` so the supervisor never deletes its own (possibly untracked) files.
  await git(cwd, ['clean', '-fd', '-e', 'kinetic'], { allowFail: true });
  await git(cwd, ['checkout', integration], { allowFail: true });
  await git(cwd, ['branch', '-D', cycleBranch], { allowFail: true });
}
...
// On restart, delete any dangling kinetic/cycle-* branches and return to integration.
export async function cleanupDanglingCycleBranches(cwd, integration) {
  const { stdout } = await git(cwd, ['branch', '--list', 'kinetic/cycle-*'], { allowFail: true });
  await git(cwd, ['checkout', integration], { allowFail: true });
  await git(cwd, ['reset', '--hard'], { allowFail: true });
  await git(cwd, ['clean', '-fd', '-e', 'kinetic'], { allowFail: true });
