...
export function emptyState() {
  return {
    ...
    // U-66: per-cycle intent/plan validation flags — set by the supervisor after the planning gate runs
    // and exposed via GET /api/cycle/state. Reset on each new cycle when the planning gate executes.
    intent_locked: false,      // true once intent-{taskId}.md is written for the current cycle
    plan_validated: null,      // true=Haiku approved, false=rejected, null=skipped/not-yet-run
    validation_feedback: '',   // last validation feedback message (empty when approved/skipped)
