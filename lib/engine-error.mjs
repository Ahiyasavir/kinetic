...
export class LockError extends EngineError {
  constructor(message, { cause, remediation: customRemediation } = {}) {
    const defaultRemediation = 'Check if another process holds the lock or if the lock file is stale. Delete the lock file only if certain no other process is running.';
    super(message, {
      code: 'LOCK_ERROR',
      cause,
      remediation: customRemediation || defaultRemediation
    });
    this.name = 'LockError';
  }
}
...
export class OperationalError extends EngineError {
  constructor(message, { cause, operation = '', remediation = '' } = {}) {
    const defaultRemediation = operation
      ? `The operation "${operation}" failed unexpectedly. Check the logs above for details and retry.`
      : 'The operation failed unexpectedly. Check the logs above for details and retry.';
    super(message, {
...
export function requireLocalPath(filePath, what) {
  if (!existsSync(filePath)) {
    throw new EngineError(
      ...
      {
        code: 'ENGINE_MISSING_PATH',
        path: filePath,
        remediation: `Restore the ${what} file from git or verify the configured path is correct. The engine cannot proceed without this resource.`
      }
    );
  }
  return filePath;
  ...
  if (err && err.code === 'ENOENT') {
    return new EngineError(
      ...
      `(a missing local code path) — not a task failure.`,
      {
        ...
        path: filePath,
        cause: err,
        remediation: `Restore or create the ${what} file. Verify the file exists at the path above and has correct permissions.`
      }
    );
  }
  return err;
