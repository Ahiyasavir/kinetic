 ...
 * @param {object} _config - resolved config (unused; quota is read from state.budget)
 * @returns UsageResult with trust='predicted'
 */
export function predictFromState(state, _config) {
  const u = state?.usage || {};
...
export function getBestUsage(state, config, snapshotPath) {
  ...
  // fiveHour: prefer the statusline (real Anthropic data) when present, but it's unavailable in the
  // engine's headless mode — so fall back to the self-tracked ledger (lib/usage-ledger.mjs) which
  // derives a real local 5h rolling window from timestamped spend. This is what gives the engine ANY
  // 5h awareness at all (previously hardcoded to null).
  ...
  if (snapFresh && snap.fiveHour?.usedPercent != null) {
    ...
    const lv = fiveHourView(state);
    if (lv.entries > 0) {
      fiveHour = lv;
      fiveHourSource = 'ledger';
      fiveHourTrust = 'medium';
    }
  }
...
export function formatUsageReport(usage) {
  ...
  if (usage.fiveHour) {
    ...
    lines.push('  (no pressure — self-tracked ledger has no spend in the last 5h)');
    lines.push('  Populates from the next merged cycle (lib/usage-ledger.mjs).');
  }
