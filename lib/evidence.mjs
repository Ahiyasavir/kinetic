...
// Entry-point filenames that are invoked directly by Node/OS, never imported by other modules.
// Declaring `wired: true` on these is incorrect (nothing imports them — that's intentional) so we
// skip the import-scan and report them as wired. This prevents false UNWIRED rejections on CLI
// entry-points, watchdogs, test runners, and install scripts.
const ENTRY_POINT_NAMES = new Set([
  'watchdog.mjs', 'supervisor.mjs', 'cli.mjs', 'night-shift.mjs',
  'run-architect-once.mjs', 'apply-architect-plan.mjs',
]);
function isEntryPoint(absPath) {
  if (ENTRY_POINT_NAMES.has(path.basename(absPath))) return true;
  // Any file inside a tests/ directory is a test runner, not a library.
  if (/[/\\]tests?[/\\]/.test(absPath)) return true;
  // Non-module data files (JSON, Markdown, YAML, etc.) are never imported as ES modules;
  // the import-wiring check is meaningless for them — treat them as always-wired.
  const ext = path.extname(absPath).toLowerCase();
  if (!['.mjs', '.js', '.cjs', '.ts', '.tsx'].includes(ext)) return true;
  // Any file with a Node shebang is a CLI script / standalone test runner.
  try {
    const head = readFileSync(absPath, 'utf8').slice(0, 80);
    if (head.startsWith('#!/')) return true;
  } catch { /* unreadable — let the existence check handle it */ }
  return false;
}
...
export function checkEvidence(task, repoRoot = REPO_ROOT, scanRoot = KINETIC_DIR) {
  ...
  for (const a of arts) {
    ...
    if (a && a.wired) {
      // Entry-point files are wired by definition — they're run directly, never imported.
      const wired = isEntryPoint(abs) || isImportedSomewhere(abs, scanRoot);
      rec.wired = wired;
