...
export async function calibrateUsage(ws, { pctUsed, quota, spentTokens, actualQuota, tier, resetInMinutes, fiveHourPctUsed, fiveHourResetInMinutes } = {}) {
  const state = await loadWsState(ws);
  ...
  // The offset absorbs everything Claude attributed to this account that the engine didn't track.
  ...
  // CRITICAL: directly overwrite measuredWeeklyTokens with the calibrated value.
  // windowSpend() prefers measuredWeeklyTokens over externalTokenOffset, so writing only the offset
  // is silently ignored — the governor keeps reading the stale JSONL-derived count. Writing the
  // calibrated figure directly makes the governor see the correct spend immediately, before the
  // next JSONL re-read can catch up. The JSONL reader will naturally converge once windowStartedAt
  // is corrected below; this just ensures the calibration takes effect on the very next cycle.
  state.usage.measuredWeeklyTokens = actualSpent;
  state.usage.measuredAt = Date.now();
  ...
  //
  // Additionally: derive windowStartedAt from resetInMinutes. If the next reset is N minutes away
  // and the window is 7 days, the current window started (7d − N) minutes ago. This corrects the
  // JSONL reader's cutoff so it only counts post-reset tokens, preventing stale pre-reset transcript
  // entries from inflating measuredWeeklyTokens on the next refresh cycle.
  let correctedResetAt = null;
  if (Number(resetInMinutes) > 0) {
    const nowMs = Date.now();
    correctedResetAt = new Date(nowMs + Number(resetInMinutes) * 60 * 1000).toISOString();
    ...
    // Back-compute when this window actually started so the JSONL reader only counts post-reset tokens.
    const intervalMs = (config.weeklyBudget?.resetIntervalDays || 7) * 86400000;
    const windowStartedMs = nowMs - (intervalMs - Number(resetInMinutes) * 60 * 1000);
    state.usage.windowStartedAt = new Date(windowStartedMs).toISOString();
  }

  // 5-HOUR (rolling session) window calibration. The Pro/Max plans enforce a SEPARATE 5-hour limit
  // that is usually the BINDING constraint (it caps far below the weekly quota). The engine only
  // reacted to it AFTER a 429; this lets the user pin its true size + reset so the pacer can avoid
  // breaching it proactively. Quota is derived: fiveHourQuota = measuredFiveHour.tokens / (pct/100).
  if (Number(fiveHourPctUsed) > 0) {
    const m5 = state.usage.measuredFiveHour || {};
    const tokens5h = Number(m5.tokens) || 0;
    if (tokens5h > 0) {
      const derivedQuota = Math.round(tokens5h / (Number(fiveHourPctUsed) / 100));
      if (Number.isFinite(derivedQuota) && derivedQuota > 0) {
        state.usage.calibratedFiveHourQuota = derivedQuota;
        state.usage.calibratedFiveHourPct = Number(fiveHourPctUsed);
      }
    }
  }
  if (Number(fiveHourResetInMinutes) > 0) {
    const resetAtMs = Date.now() + Number(fiveHourResetInMinutes) * 60 * 1000;
    state.usage.measuredFiveHour = { ...(state.usage.measuredFiveHour || {}), resetAtMs, msLeft: Number(fiveHourResetInMinutes) * 60 * 1000 };
  }
