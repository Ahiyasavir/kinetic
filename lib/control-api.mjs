...
import { computeCostAnalytics } from './costAnalytics.mjs';
...
function effectiveGov(state, config, budgetScope) {
  ...
  if (p && typeof p.spent === 'number' && typeof p.action === 'string') {
    // Re-derive spent from current usage counters so the display stays fresh between cycles.
...
// completed cycle. Side-effect-free read; empty/zeroed when the workspace has never run. ──
export async function getCostAnalytics(ws) {
  const state = await loadWsState(ws);
  return { workspace: ws.id, ...computeCostAnalytics(state ? state.stats : null) };
}
...
export async function getUsage(ws, config = engineConfig) {
  ...
  const base = state || { usage: {} };
  const gov = effectiveGov(state, config, ws.budgetScope);
  const fiveHour = fiveHourView(base);                 // self-tracked rolling 5h window
  ...
  return {
    ...
    sevenDay: {
      ...
      resetAt: base.usage?.windowResetAt || null,
...
//   pctUsed     — the % shown on Claude's usage page (e.g. 48)
...
export async function calibrateUsage(ws, { pctUsed, quota, spentTokens } = {}) {
  const state = await loadWsState(ws);
  ...
  // Resolve the quota: user-supplied → persisted from last governor run → hard fallback
  const knownQuota = Number(quota) > 0 ? Math.round(Number(quota))
    : (state.budget?.quota || null);
  ...
  const calibConfig = knownQuota
    ? { ...engineConfig, budgetGovernor: { ...(engineConfig.budgetGovernor || {}), weeklyTokenQuota: knownQuota } }
    : engineConfig;
