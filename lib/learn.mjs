...
// TOKEN-AWARE adaptive cadence — the "smart" replacement for a fixed maxCyclesPerDay.
//
// computeVelocityFactor/effectivePerDay above pace by CYCLE COUNT (cycles ÷ a cycle budget), which is
// blind to how many TOKENS each cycle actually burns — that is exactly how the loop reached 63% of the
// weekly TOKEN quota while the cycle-based pacer still reported "on budget". This derives the per-day
// cycle allowance directly from the budget that actually matters:
//
//   affordableTotalCycles = (remainingUsableTokens / avgTokensPerCycle) × safetyFactor
//   perDay                = floor(affordableTotalCycles / daysToReset)
//
// So as tokens deplete the cadence auto-tightens, and after the weekly reset (full quota again) it
// relaxes back toward the ceiling — no manual retuning. Result is clamped to [floorPerDay, ceilPerDay]
// (ceilPerDay = the configured maxCyclesPerDay, now acting as an upper bound rather than a flat cap).
//
//   remainingUsableTokens — quota×(1−reserve) − spent (pass Infinity / NaN when no quota is known)
//   daysToReset           — wall-clock days left in the window (floored at 1h to avoid a near-reset spike)
//   avgTokensPerCycle     — observed spent/cycles (or the governor's per-cycle estimate on a fresh window)
export function computeAdaptiveCyclesPerDay({
  remainingUsableTokens, daysToReset, avgTokensPerCycle,
  floorPerDay = 4, ceilPerDay = 200, safetyFactor = 1.0
} = {}) {
  const ceil = Math.max(1, Number(ceilPerDay) || 200);
  const floor = Math.min(ceil, Math.max(1, Number(floorPerDay) || 4));
  const remaining = Number(remainingUsableTokens);
  // No discoverable token quota → no extra throttle (fall back to the ceiling = legacy behavior).
  if (!Number.isFinite(remaining)) return ceil;
  // Budget exhausted → minimum cadence.
  if (remaining <= 0) return floor;
  const perCycle = Math.max(1, Number(avgTokensPerCycle) || 1);
  // Floor daysToReset at 1 hour so a near-reset window can't divide the allowance into a huge spike.
  const days = Math.max(Number(daysToReset) || 0, 1 / 24);
  const affordableTotal = (remaining / perCycle) * (Number(safetyFactor) || 1.0);
  const perDay = Math.floor(affordableTotal / days);
  return clamp(perDay, floor, ceil);
}
