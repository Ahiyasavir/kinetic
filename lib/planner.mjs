...
import { validatePlan } from './planValidator.mjs';
...
export async function runPlanner(task, handoffDir, config, invoker, log = () => {}) {
  ...
  try {
    ...
    // Validate the plan against the intent (the cheap Haiku gate — via planValidator.mjs).
    let verdict = await validatePlan(intentMd, planText, invoker, model);
    ...
    if (verdict && verdict.verdict === 'REJECT') {
      ...
      verdict = await validatePlan(intentMd, planText, invoker, model);
