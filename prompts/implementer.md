...
{{REVISION_BLOCK}}
{{APPLICABLE_LESSONS}}
## Test Suite (Generated by Tester Phase)
{{TEST_FILE_PATH}}

**⚠️ CRITICAL: If a test file path is shown above, you MUST:**
1. **Run the tests** using `npm test -- {{TEST_FILE_PATH}}`
2. **Report the outcome in your handoff** — include `testResults` in the JSON with `passed: true/false` and any error messages
3. **Do NOT mark the cycle ready** if tests fail — if `npm test` returns non-zero exit code, you MUST fail the implementation and provide the exact failure output in `testResults.output`

The tests define what "done" means at specification time — they are not optional. A passing test suite is a gate to moving forward.
